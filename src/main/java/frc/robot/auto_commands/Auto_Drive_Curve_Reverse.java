 ...
 *
 */
public class Auto_Drive_Curve_Reverse extends Command {
	double distance;
    ...
    public Auto_Drive_Curve_Reverse(double angle, double radius,  double power, double minpower, double rampup, double rampdown) {
        ...
        this.distance = Math.abs(distance * Robot.drive.getratio_high());
        this.angle = angle;
        this.power = power;
    ...
    protected void execute() {
    	...
    	//currPos =  Math.abs(Robot.drive.getDistance()) ; 	
		currPos =  Math.abs(Robot.drive.getDistance_Left()) ; 	
    	...
    	SmartDashboard.putNumber("currPosCurve", currPos);
    ...
    protected boolean isFinished() {
		...
		return (( (Math.abs(distance) < Math.abs(currPos)) )) || isTimedOut();
